...
public class ByteBufExamples {
    ...
    public void heapBuffer() {
        ...
        if (heapBuf.hasArray()) {
            ...
            Assert.assertEquals(array.length, 1024);
            array[0] = 1;
            /**
             * heapBuf.arrayOffset(): 返回此缓冲区的后备字节数组中第一个字节的偏移量
             * heapBuf.readerIndex(): 该Buffer的读索引
             *
             * heapBuf.readableBytes(): 返回可读字节数，它等于this.writerIndex - his.readerIndex。
             */
            int offset = heapBuf.arrayOffset() + heapBuf.readerIndex();
            int readableLength = heapBuf.readableBytes();
            int writableLength = heapBuf.writableBytes();
            System.out.println("offset====>" + offset);
            System.out.println("readableLength====>" + readableLength);
            System.out.println("writableLength====>" + writableLength);
            handleArray(array, offset, readableLength);
        }
    }
