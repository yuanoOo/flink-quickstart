...
/**
  ...
  * @time 下午4:25
  */
class BloomFilterInValueState extends RichFilterFunction[(Long, Long)]{
    ...
    override def filter(value: (Long, Long)): Boolean = {
        if (sum.value() == null){
            val bloomFilter: BloomFilter[String] = BloomFilter.create(new Funnel[String] {override def funnel(from: String, into: Sink): Unit = into.putString(from, Charsets.UTF_8)}, EXPECTED_INSERTIONS, BLOOM_FPP)
